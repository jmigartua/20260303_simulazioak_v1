...
**Current implementation state:** `v0.1.2` (stable), CI green, 110/110 tests passing
**Scope note:** Synced to main through commit 60 (2026-03-04). Re-validate matrix rows after newer commits.
...
| R2 | Per-module testing | Every implemented module has unit tests; boundaries have contract tests | D14 | `tests/core/`, `tests/contracts/`, `tests/scenarios/`, `tests/app/`, `tests/tooling/`, `tests/integration/`, `tests/adapters/` | CI pytest (`110/110`) | Testing chapter matrix by package | Done (implemented scope) |
| R3 | Configurable agent attributes | Agent attributes and behavior parameters are configurable in headless runtime via validated JSON specs; UI editor pending | D6, D7, D15 | `sim_framework/contracts/validators.py`, `sim_framework/app/cli.py` (`--agent-spec-json`), `sim_framework/scenarios/ants_foraging/spec.py`, `sim_framework/scenarios/drone_patrol/spec.py`, `Plans/r3_agent_spec_reproducibility_2026-03-04.md` | `tests/contracts/test_validators_schema.py`, `tests/app/test_cli_runtime_mode.py`, `tests/scenarios/test_ants_scenario_loads.py`, `tests/scenarios/test_drone_scenario_loads.py` | Custom spec reproducibility artifacts (`Plans/r3_agent_spec_*`, `Plans/r3_*_custom_*.json`, `Plans/runs/r3-*-custom-20260304/run.json`) | Done (implemented scope) |
...
| R4 | Configurable physics | Physics is parameterized in core/runtime (boundary mode configurable at app runtime); UI editor pending | D6, D15 | `sim_framework/core/physics.py`, `sim_framework/app/cli.py` (`--boundary-mode`), `sim_framework/scenarios/ants_foraging/spec.py`, `sim_framework/scenarios/drone_patrol/spec.py`, `Plans/r4_physics_reproducibility_2026-03-04.md` | `tests/core/test_physics_movement.py`, `tests/app/test_cli_runtime_mode.py`, `tests/scenarios/test_ants_scenario_loads.py`, `tests/scenarios/test_drone_scenario_loads.py` | Boundary-mode reproducibility artifacts (`Plans/r4_physics_*`, `Plans/runs/r4-physics-*`) | Done (implemented scope) |
...
| R6 | Modern, performant UI | Web UI/PixiJS not yet implemented in this release line; scoped roadmap is defined for next increment | D1, D13 | `Plans/r6_r9_ui_desktop_roadmap_2026-03-04.md` | N/A (implementation pending) | Roadmap milestones and acceptance criteria documented for UI line | In progress |
| R7 | Playback and capture controls incl. rewind | Play/pause/step/reset/seek/rewind and save/load run bundles implemented headlessly; screenshot capture remains UI-adapter scope | D8, D9, D10 | `sim_framework/core/engine.py`, `sim_framework/core/history.py`, control commands in contracts, `sim_framework/adapters/persistence/json_file.py`, `sim_framework/app/cli.py`, `Plans/r7_reproducibility_example_2026-03-04.md` | determinism + history + command tests, `tests/adapters/test_json_file_persistence.py`, `tests/app/test_cli_runtime_mode.py` | Adapter-backed save/load reproducibility artifacts (`Plans/r7_repro_*.json`, `Plans/runs/r7-repro-20260304/run.json`) | Done (implemented scope) |
...
| R9 | Linux desktop + web-like app | Browser/desktop shell path remains pending; Linux packaging path planned with explicit milestones | D1 | `sim_framework/app/cli.py` (headless composition root), `Plans/r6_r9_ui_desktop_roadmap_2026-03-04.md` | CLI smoke tests (current baseline) | Desktop/web milestone plan and packaging acceptance gates | In progress |
