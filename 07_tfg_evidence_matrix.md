...
**Current implementation state:** `v0.1.2` (stable), CI green, 106/106 tests passing
**Scope note:** Synced to main through commit 59 (2026-03-04). Re-validate matrix rows after newer commits.
...
| R2 | Per-module testing | Every implemented module has unit tests; boundaries have contract tests | D14 | `tests/core/`, `tests/contracts/`, `tests/scenarios/`, `tests/app/`, `tests/tooling/`, `tests/integration/`, `tests/adapters/` | CI pytest (`106/106`) | Testing chapter matrix by package | Done (implemented scope) |
